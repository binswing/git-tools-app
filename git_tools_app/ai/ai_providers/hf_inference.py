...
def get_models():
    """Fetch available Hugging Face models from the API."""
    url = MODELS_URL
    params = {"pipeline_tag": "text-generation", "sort": "trending", "limit": 15}
...
def generate_message(model, system_prompt, user_prompt):
    """
    Generate a message using the Hugging Face Inference API.
    ...
    payload = {"inputs": prompt, "parameters": {"max_new_tokens": 100, "return_full_text": False, "temperature": 0.2}}
    ...
    try:
        ...
        details = getattr(e.response, "text", "No response body available.")
        logger.error(f"Hugging Face API Error: {status}")
        logger.debug(f"HF Error Details: {details}")
